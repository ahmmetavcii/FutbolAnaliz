...
python ~/projects/football-analytics/scripts/run_pipeline.py \
  --config ~/projects/football-analytics/configs/pipeline/mvp1_tracking.yaml \
  --input /home/ahmet/workspace/staging/test_clip.mp4
```

Log directory: `/home/ahmet/logs/football_setup_20260717_234122`

## 13. Real-video readiness verification (2026-07-18)

**Verdict:** PASS for GPU detection + tracking on a real football clip via the football-analytics MVP-1 pipeline.

| Check | Result |
|---|---|
| `check_env.py` | PASS |
| `pip check` | clean |
| CUDA FP16 matmul + YOLO nano inference | PASS |
| Demo video source | `/home/ahmet/projects/third-party/no-bells-just-whistles/examples/iniesta_sample.mp4` (pnlcalib copy was 2 bytes / invalid) |
| Copied to SSD | `/mnt/c/football_data/videos/test_clips/iniesta_sample.mp4` |
| Staging clip | `/home/ahmet/workspace/staging/test_clip.mp4` (full 6.12 s; shorter than 10–20 s target because source is only 6.12 s; not padded) |
| OpenCV / FFprobe | PASS (1920x1080 @ 25 fps, 153 frames) |
| Pipeline run | `run_20260718_011237_ef8a93` **PASS** |
| Output validation | `validate_run_outputs.py` **PASS** |

| Stage | FPS | Seconds | Peak VRAM | Notes |
|---|---|---|---|---|
| Detection (yolo11n, imgsz=640, batch=1, half=True, device=0) | 17.48 | 8.75 | 50.8 MiB | 1997 detections (1988 person, 9 sports ball) |
| ByteTrack | 32.92 | 4.65 | 56.3 MiB | 1621 rows, 53 track IDs — **primary** → `tracks.parquet` / `annotated_video.mp4` |
| BoT-SORT | 34.01 | 4.50 | 54.6 MiB | 1608 rows, 55 track IDs |
| Full pipeline wall time | — | 25.05 | — | ingest + detect + both trackers + MP4 remux |

sn-nvs remains **BLOCKED_BUILD** (CUDA Toolkit intentionally not installed).
